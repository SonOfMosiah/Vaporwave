...
describe("Router", function () {
  ...
  it("setGov", async () => {
    await expect(
      router.connect(user0).setGov(user1.address)
  ...
  it("addPlugin", async () => {
    await expect(
      router.connect(user0).addPlugin(user1.address)
    ).to.be.revertedWith("Router: forbidden");
  ...
  it("removePlugin", async () => {
    await expect(
      router.connect(user0).removePlugin(user1.address)
    ).to.be.revertedWith("Router: forbidden");
  ...
  it("approvePlugin", async () => {
    expect(await router.approvedPlugins(user0.address, user1.address)).eq(
      false
    );
  ...
  it("pluginTransfer", async () => {
    ...
    await router
      .connect(user1)
      .pluginTransfer(dai.address, user0.address, user2.address, 800);
    ...
    await expect(
      router
        .connect(user2)
        .pluginTransfer(dai.address, user0.address, user2.address, 1)
  ...
  it("swap, buy USDV", async () => {
    ...
    const tx = await router
      ...
      .swap(
        [dai.address, usdv.address],
        expandDecimals(200, 18),
        expandDecimals(199, 18),
        user0.address
      );
    await reportGasUsed(tx, "buyUSDV gas used");
    expect(await dai.balanceOf(user0.address)).eq(0);
    expect(await usdv.balanceOf(user0.address)).eq("199400000000000000000"); // 199.4
  });

  it("swap, sell USDV", async () => {
    await dai.mint(user0.address, expandDecimals(200, 18));
    await dai.connect(user0).approve(router.address, expandDecimals(200, 18));
    ...
    await usdv.connect(user0).approve(router.address, expandDecimals(100, 18));
    await expect(
      router
        .connect(user0)
        .swap(
    ...
    expect(await usdv.balanceOf(user0.address)).eq("99400000000000000000"); // 99.4
  });
  ...
  it("swap, path.length == 2", async () => {
    ...
    await expect(
      router
        .connect(user0)
        .swap(
          [btc.address, usdv.address],
          expandDecimals(1, 8),
          expandDecimals(60000, 18),
          user0.address
        )
    ).to.be.revertedWith("Router: insufficient amountOut");
    await router
  ...
  it("swap, path.length == 3", async () => {
    ...
    await usdv
      .connect(user0)
      .approve(router.address, expandDecimals(20000, 18));

    expect(await dai.balanceOf(user0.address)).eq(0);
    expect(await usdv.balanceOf(user0.address)).eq(expandDecimals(89730, 18));
    await expect(
      router
        .connect(user0)
        .swap(
          [usdv.address, dai.address, usdv.address],
    ...
    expect(await dai.balanceOf(user0.address)).eq(0);
    expect(await usdv.balanceOf(user0.address)).eq("89610180000000000000000"); // 89610.18

    await usdv
      .connect(user0)
      .approve(router.address, expandDecimals(40000, 18));
    await expect(
      router
        .connect(user0)
        .swap(
          [usdv.address, dai.address, btc.address],
          expandDecimals(30000, 18),
    ...
    await reportGasUsed(tx, "swap gas used");
    expect(await usdv.balanceOf(user0.address)).eq("69610180000000000000000"); // 69610.18
  ...
  it("decreasePositionAndSwapETH", async () => {
    ...
    await router
      .connect(user0)
      .swap(
        [bnb.address, usdv.address],
        expandDecimals(10, 18),
        expandDecimals(2900, 18),
        user0.address
      );
    ...
    expect(await ethers.provider.getBalance(wallet0.address)).eq(0);
    ...
    expect(await ethers.provider.getBalance(wallet0.address)).eq(
      "654696002000000000"
    ); // 0.654696002
    expect(await ethers.provider.getBalance(router.address)).eq(0);
  });
