...
describe("OrderBook, decrease position orders", () => {
  ...
  it("Execute decrease order, invalid price", async () => {
    ...
    let triggerPrice, isLong, triggerAboveThreshold, newBtcPrice, setPriceTwice;
    let orderIndex = 0;
    ...
    for ([
      ...
      const order = await orderBook.decreaseOrders(
  ...
  it("Execute decrease order, long", async () => {
    await btc
      .connect(defaults.user)
      .transfer(vault.address, expandDecimals(10000, 8).div(BTC_PRICE));
    await vault
      .connect(defaults.user)
      .increasePosition(
        defaults.user.address,
    ...
    const tx = await orderBook.executeDecreaseOrder(
      defaults.user.address,
      0,
      user1.address
    );
  ...
  it("Execute decrease order, short, BTC", async () => {
    ...
    expect(daiBalanceAfter.sub(daiBalanceBefore)).to.be.equal(
      "10460000000000000000000"
    );
  ...
  it("Execute decrease order, long, BNB", async () => {
    ...
    expect(position.size, "position.size").to.be.equal(
      "1500000000000000000000000000000000"
    );
    expect(position.collateral, "position.collateral").to.be.equal(
      "748500000000000000000000000000000"
    );
