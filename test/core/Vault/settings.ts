...
import { initVault, getBnbConfig } from "./helpers";
...
describe("Vault.settings", function () {
  ...
  beforeEach(async () => {
    ...
    const contracts = await initVault(vault, router, usdv, vaultPriceFeed);
  ...
  it("inits", async () => {
    expect(await usdv.gov()).eq(wallet.address);
    expect(await usdv.vaults(vault.address)).eq(true);
    expect(await usdv.vaults(user0.address)).eq(false);
    ...
    expect(await vault.usdv()).eq(usdv.address);
  ...
  it("setVaultUtils", async () => {
    await expect(
      vault.connect(user0).setVaultUtils(user1.address)
  ...
  it("setMaxGlobalShortSize", async () => {
    await expect(
      vault.connect(user0).setMaxGlobalShortSize(bnb.address, 1000)
    ).to.be.revertedWith("Vault: forbidden");
  ...
  it("setInManagerMode", async () => {
    await expect(
      vault.connect(user0).setInManagerMode(true)
    ).to.be.revertedWith("Vault: forbidden");
  ...
  it("setManager", async () => {
    await expect(
      vault.connect(user0).setManager(user1.address, true)
    ).to.be.revertedWith("Vault: forbidden");
  ...
  it("setInPrivateLiquidationMode", async () => {
    await expect(
      vault.connect(user0).setInPrivateLiquidationMode(true)
    ).to.be.revertedWith("Vault: forbidden");
  ...
  it("setIsSwapEnabled", async () => {
    await expect(
      vault.connect(user0).setIsSwapEnabled(false)
    ).to.be.revertedWith("Vault: forbidden");
  ...
  it("setIsLeverageEnabled", async () => {
    await expect(
      vault.connect(user0).setIsLeverageEnabled(false)
    ).to.be.revertedWith("Vault: forbidden");
  ...
  it("setMaxGasPrice", async () => {
    await expect(vault.connect(user0).setMaxGasPrice(20)).to.be.revertedWith(
      "Vault: forbidden"
    );
  ...
  it("setGov", async () => {
    await expect(vault.connect(user0).setGov(user1.address)).to.be.revertedWith(
      "Vault: forbidden"
    );
  ...
  it("setPriceFeed", async () => {
    await expect(
      vault.connect(user0).setPriceFeed(user1.address)
    ).to.be.revertedWith("Vault: forbidden");
  ...
  it("setMaxLeverage", async () => {
    await expect(vault.connect(user0).setMaxLeverage(10000)).to.be.revertedWith(
      "Vault: forbidden"
    );
  ...
  it("setBufferAmount", async () => {
    await expect(
      vault.connect(user0).setBufferAmount(bnb.address, 700)
    ).to.be.revertedWith("Vault: forbidden");
  ...
  it("setFundingRate", async () => {
    await expect(
      vault.connect(user0).setFundingRate(59 * 60, 10001, 10001)
    ).to.be.revertedWith("Vault: forbidden");
    ...
    await expect(
      vault.connect(user0).setFundingRate(60 * 60, 10001, 10001)
    ).to.be.revertedWith("Vault: invalid _fundingRateFactor");

    await expect(
      vault.connect(user0).setFundingRate(60 * 60, 10000, 10001)
  ...
  it("setTokenConfig", async () => {
    ...
    await expect(
      vault.connect(user0).setTokenConfig(...params)
    ).to.be.revertedWith("Vault: forbidden");

    await expect(vault.setTokenConfig(...params)).to.be.revertedWith(
      "VaultPriceFeed: could not fetch price"
    );
    ...
    expect(await vault.maxUsdvAmounts(bnb.address)).eq(0);
    ...
    expect(await vault.maxUsdvAmounts(dai.address)).eq(1000);
    ...
    expect(await vault.maxUsdvAmounts(dai.address)).eq(500);
  ...
  it("clearTokenConfig", async () => {
    ...
    expect(await vault.minProfitBasisPoints(bnb.address)).eq(75);
    expect(await vault.maxUsdvAmounts(bnb.address)).eq(500);
    expect(await vault.stableTokens(bnb.address)).eq(true);
    expect(await vault.shortableTokens(bnb.address)).eq(true);
    ...
    await expect(
      vault.connect(user0).clearTokenConfig(bnb.address)
    ).to.be.revertedWith("Vault: forbidden");
  ...
  it("setUsdvAmount", async () => {
    ...
    await expect(
      vault.connect(user0).setUsdvAmount(bnb.address, 50000)
    ).to.be.revertedWith("Vault: forbidden");

    await vault.setGov(user0.address);

    expect(await vault.usdvAmounts(bnb.address)).eq(29700);
    await vault.connect(user0).setUsdvAmount(bnb.address, 50000);
    expect(await vault.usdvAmounts(bnb.address)).eq(50000);

    await vault.connect(user0).setUsdvAmount(bnb.address, 10000);
    expect(await vault.usdvAmounts(bnb.address)).eq(10000);
  });
  ...
  it("upgradeVault", async () => {
    ...
    await expect(
      vault.connect(user0).upgradeVault(user1.address, bnb.address, 1000)
    ).to.be.revertedWith("Vault: forbidden");
  ...
  it("setErrorController", async () => {
    const vaultErrorController = await deployContract(
      "VaultErrorController",
      []
    );
    await expect(
      vaultErrorController.setErrors(vault.address, [
        "Example Error 1",
        "Example Error 2",
      ])
    ...
    await expect(
      vault.connect(user0).setErrorController(vaultErrorController.address)
    ).to.be.revertedWith("Vault: forbidden");
    ...
    await expect(
      vaultErrorController
        .connect(user0)
        .setErrors(vault.address, ["Example Error 1", "Example Error 2"])
    ).to.be.revertedWith("Governable: forbidden");

    await vaultErrorController.setErrors(vault.address, [
      "Example Error 1",
      "Example Error 2",
    ]);
