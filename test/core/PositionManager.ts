...
describe("PositionManager", function () {
  ...
  it("decreasePositionAndSwap and decreasePositionAndSwapETH", async () => {
    ...
    const bnbBalance = await ethers.provider.getBalance(user0.address);
    await positionManager
      .connect(user0)
      .decreasePositionAndSwapETH(...params, 0);
    expect((await ethers.provider.getBalance(user0.address)).gt(bnbBalance)).to
      .be.true;
  ...
  it("executeIncreaseOrder", async () => {
    ...
    await createIncreaseOrder(expandDecimals(100, 18), BigNumber.from("0"));
