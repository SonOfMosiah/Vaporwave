...
import { expandDecimals, reportGasUsed, newWallet } from "../shared/utilities";
...
describe("PositionRouter", function () {
  ...
  let wallet: any,
    ...
    tokenManager: any,
    mintReceiver: any;
  const depositFee: number = 50;
  const minExecutionFee: number = 4000;
  let vault: any;
  let timelock: any;
  let usdv: any;
  let router: any;
  let positionRouter: any;
  let referralStorage: any;
  ...
  it("setAdmin", async () => {
    await expect(
      positionRouter.connect(user0).setAdmin(user1.address)
  ...
  it("setDepositFee", async () => {
    await expect(
      positionRouter.connect(user0).setDepositFee(25)
  ...
  it("setIncreasePositionBufferBps", async () => {
    await expect(
      positionRouter.connect(user0).setIncreasePositionBufferBps(200)
    ).to.be.revertedWith("BasePositionManager: forbidden");
  ...
  it("setReferralStorage", async () => {
    await expect(
      positionRouter.connect(user0).setReferralStorage(user1.address)
    ).to.be.revertedWith("BasePositionManager: forbidden");
  ...
  it("setMaxGlobalSizes", async () => {
    ...
    await expect(
      positionRouter
        .connect(user0)
        .setMaxGlobalSizes(tokens, maxGlobalLongSizes, maxGlobalShortSizes)
    ).to.be.revertedWith("BasePositionManager: forbidden");
  ...
  it("withdrawFees", async () => {
    ...
    await vault.buyUSDV(bnb.address, user1.address);
    ...
    await positionRouter
      .connect(user0)
      .createIncreasePosition(...params.concat([4000, referralCode]), {
        value: 4000,
      });
    await positionRouter
      .connect(positionKeeper)
      .executeIncreasePosition(key, executionFeeReceiver.address);
    expect(await ethers.provider.getBalance(executionFeeReceiver.address)).eq(
      4000
    );
    ...
    expect(await positionRouter.feeReserves(bnb.address)).eq(
      "9970000000000000"
    ); // 0.00997

    await expect(
      positionRouter.connect(user2).withdrawFees(dai.address, user3.address)
    ).to.be.revertedWith("BasePositionManager: forbidden");
    ...
    await positionRouter
      .connect(user2)
      .withdrawFees(dai.address, user3.address);
    ...
    await positionRouter
      .connect(user2)
      .withdrawFees(bnb.address, user3.address);
  ...
  it("approve", async () => {
    await expect(
      positionRouter.connect(user0).approve(bnb.address, user1.address, 100)
    ).to.be.revertedWith("Governable: forbidden");
    ...
    await positionRouter
      .connect(user0)
      .approve(bnb.address, user1.address, 100);
    expect(await bnb.allowance(positionRouter.address, user1.address)).eq(100);
  });

  it("sendValue", async () => {
    await expect(
      positionRouter.connect(user0).sendValue(user1.address, 0)
    ).to.be.revertedWith("Governable: forbidden");
  ...
  it("setPositionKeeper", async () => {
    await expect(
      positionRouter.connect(user0).setPositionKeeper(user1.address, true)
    ).to.be.revertedWith("BasePositionManager: forbidden");
  ...
  it("setMinExecutionFee", async () => {
    await expect(
      positionRouter.connect(user0).setMinExecutionFee("7000")
    ).to.be.revertedWith("BasePositionManager: forbidden");
  ...
  it("setIsLeverageEnabled", async () => {
    await expect(
      positionRouter.connect(user0).setIsLeverageEnabled(false)
    ).to.be.revertedWith("BasePositionManager: forbidden");
  ...
  it("setDelayValues", async () => {
    await expect(
      positionRouter.connect(user0).setDelayValues(7, 21, 600)
    ).to.be.revertedWith("BasePositionManager: forbidden");
  ...
  it("setRequestKeysStartValues", async () => {
    await expect(
      positionRouter.connect(user0).setRequestKeysStartValues(5, 8)
    ).to.be.revertedWith("BasePositionManager: forbidden");
  ...
  it("validateExecution", async () => {
    ...
    expect(await vault.guaranteedUsd(bnb.address)).eq(
      "5407800000000000000000000000000000"
    );
  ...
  it("decreasePosition acceptablePrice long", async () => {
    ...
    await positionRouter
      .connect(user0)
      .createDecreasePosition(...decreasePositionParams.concat([4000, false]), {
        value: 4000,
      });
  ...
  it("maxGlobalShortSize", async () => {
    ...
    await positionRouter
      .connect(positionKeeper)
      .executeIncreasePosition(key, executionFeeReceiver.address);
    expect(await vault.globalShortSizes(bnb.address)).eq(
  ...
  it("createIncreasePosition, executeIncreasePosition, cancelIncreasePosition", async () => {
    ...
    expect(await ethers.provider.getBalance(positionRouter.address)).eq(0);
  ...
  it("createIncreasePositionETH, executeIncreasePosition, cancelIncreasePosition", async () => {
    ...
    await expect(
      positionRouter
        .connect(user0)
        .createIncreasePositionETH(...params.concat([3000, referralCode]))
    ).to.be.revertedWith("PositionRouter: invalid executionFee");

    await expect(
      positionRouter
        .connect(user0)
        .createIncreasePositionETH(...params.concat([4000, referralCode]), {
          value: 3000,
        })
    ).to.be.revertedWith("PositionRouter: invalid msg.value");
    ...
    const tx = await positionRouter
      .connect(user0)
      .createIncreasePositionETH(...params.concat([4000, referralCode]), {
        value: expandDecimals(1, 18).add(4000),
      });
    ...
    expect(await ethers.provider.getBalance(positionRouter.address)).eq(0);
    expect(
      (await ethers.provider.getBalance(user1.address)).sub(balanceBefore)
  ...
  it("createIncreasePosition, createDecreasePosition, executeDecreasePosition, cancelDecreasePosition", async () => {
    ...
    await expect(
      positionRouter
        .connect(user0)
        .createDecreasePosition(...decreasePositionParams.concat([3000, false]))
    ).to.be.revertedWith("PositionRouter: invalid executionFee");
    ...
    await expect(
      positionRouter
        ...
        .createDecreasePosition(
          ...decreasePositionParams.concat([4000, false]),
          { value: 3000 }
        )
    ).to.be.revertedWith("PositionRouter: invalid msg.value");
    ...
    await expect(
      positionRouter
        .connect(user0)
        .createDecreasePosition(
          ...decreasePositionParams.concat([4000, true]),
          { value: 4000 }
        )
    ).to.be.revertedWith("PositionRouter: invalid _path");
    ...
    let decreaseRequestPath =
      await positionRouter.getDecreasePositionRequestPath(key);
    ...
    await positionRouter
      .connect(user0)
      .createDecreasePosition(...decreasePositionParams.concat([4000, true]), {
        value: 4000,
      });
    ...
    expect(await ethers.provider.getBalance(collateralReceiver.address)).eq(0);
    ...
    expect(await ethers.provider.getBalance(collateralReceiver.address)).eq(
      "496666666666666666"
    ); // 0.496666666666666666
    ...
    await positionRouter
      .connect(positionKeeper)
      .cancelDecreasePosition(key, executionFeeReceiver.address);
    ...
    await positionRouter
      ...
      .createIncreasePosition(
        ...increasePositionParams.concat([4000, referralCode]),
        { value: 4000 }
      );
    key = await positionRouter.getRequestKey(user0.address, 2);
    ...
    expect(await ethers.provider.getBalance(collateralReceiver1.address)).eq(0);
    await positionRouter
      .connect(positionKeeper)
      .executeDecreasePosition(key, executionFeeReceiver.address);
    expect(await ethers.provider.getBalance(collateralReceiver1.address)).eq(
      "496838333333333333"
    ); // 0.496838333333333333
  });
  ...
  it("executeIncreasePositions, executeDecreasePositions", async () => {
    ...
    await expect(
      positionRouter
        .connect(positionKeeper)
        .executeIncreasePositions(100, executionFeeReceiver.address)
    ).to.be.revertedWith("PositionRouter: forbidden");

    await expect(
      positionRouter
        .connect(positionKeeper)
        .executeDecreasePositions(100, executionFeeReceiver.address)
    ).to.be.revertedWith("PositionRouter: forbidden");
    ...
    await positionRouter
      .connect(positionKeeper)
      .executeIncreasePosition(key2, executionFeeReceiver.address);
    expect(await ethers.provider.getBalance(executionFeeReceiver.address)).eq(
      4000
    );

    expect((await positionRouter.increasePositionRequests(key2)).account).eq(
      AddressZero
    );
    ...
    await expect(
      positionRouter
        .connect(positionKeeper)
        .executeIncreasePosition(key3, executionFeeReceiver.address)
    ...
    await positionRouter
      .connect(positionKeeper)
      .executeIncreasePositions(0, executionFeeReceiver.address);
    expect((await positionRouter.increasePositionRequests(key0)).account).eq(
      user0.address
    );
    expect((await positionRouter.increasePositionRequests(key1)).account).eq(
      user1.address
    );
    expect((await positionRouter.increasePositionRequests(key2)).account).eq(
      AddressZero
    );
    expect((await positionRouter.increasePositionRequests(key3)).account).eq(
      user3.address
    );
    expect((await positionRouter.increasePositionRequests(key4)).account).eq(
      user4.address
    );
    ...
    await positionRouter
      .connect(positionKeeper)
      .executeIncreasePositions(1, executionFeeReceiver.address);
    expect((await positionRouter.increasePositionRequests(key0)).account).eq(
      AddressZero
    );
    expect((await positionRouter.increasePositionRequests(key1)).account).eq(
    ...
    await positionRouter
      .connect(positionKeeper)
      .executeIncreasePositions(10, executionFeeReceiver.address);
    ...
    await positionRouter
      .connect(positionKeeper)
      .executeIncreasePositions(6, executionFeeReceiver.address);
    ...
    expect(
      (await positionRouter.decreasePositionRequests(decreaseKey0)).account
    ).eq(user0.address);
    ...
    expect(
      (await positionRouter.decreasePositionRequests(decreaseKey1)).account
    ).eq(user1.address);
    ...
    expect(
      (await positionRouter.decreasePositionRequests(decreaseKey2)).account
    ).eq(user2.address);
    ...
    expect(
      (await positionRouter.decreasePositionRequests(decreaseKey3)).account
    ).eq(user3.address);
    ...
    expect(
      (await positionRouter.decreasePositionRequests(decreaseKey4)).account
    ).eq(user4.address);
    ...
    await positionRouter
      .connect(positionKeeper)
      .executeDecreasePositions(0, executionFeeReceiver.address);
    expect(
      (await positionRouter.decreasePositionRequests(decreaseKey0)).account
    ).eq(user0.address);
    expect(
    ...
    expect(await positionRouter.decreasePositionRequestKeys(0)).eq(
      decreaseKey0
    );
    expect(await positionRouter.decreasePositionRequestKeys(1)).eq(
      decreaseKey1
    );
    expect(await positionRouter.decreasePositionRequestKeys(2)).eq(
      decreaseKey2
    );
    expect(await positionRouter.decreasePositionRequestKeys(3)).eq(
      decreaseKey3
    );
    expect(await positionRouter.decreasePositionRequestKeys(4)).eq(
      decreaseKey4
    );
    ...
    await positionRouter
      .connect(positionKeeper)
      .executeDecreasePositions(1, executionFeeReceiver.address);
    expect(
      (await positionRouter.decreasePositionRequests(decreaseKey0)).account
    ).eq(AddressZero);
    expect(
    ...
    await positionRouter
      .connect(positionKeeper)
      .executeDecreasePositions(10, executionFeeReceiver.address);
    expect(
      (await positionRouter.decreasePositionRequests(decreaseKey0)).account
    ).eq(AddressZero);
    expect(
    ...
    await positionRouter
      .connect(positionKeeper)
      .executeDecreasePositions(6, executionFeeReceiver.address);
