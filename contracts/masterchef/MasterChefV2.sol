...
//             uint256 triReward = blocks.mul(triPerBlock()).mul(pool.allocPoint) /
//                 totalAllocPoint;
//             accTriPerShare = accTriPerShare.add(
//                 triReward.mul(ACC_TRI_PRECISION) / lpSupply
//             );
//         }
//         pending = int256(user.amount.mul(accTriPerShare) / ACC_TRI_PRECISION)
//             .sub(user.rewardDebt)
//             .toUInt256();
//     }
...
//         amount =
//             uint256(MASTER_CHEF.triPerBlock()).mul(
...
//             if (lpSupply > 0) {
//                 uint256 blocks = block.number.sub(pool.lastRewardBlock);
//                 uint256 triReward = blocks.mul(triPerBlock()).mul(
//                     pool.allocPoint
//                 ) / totalAllocPoint;
//                 pool.accTriPerShare = pool.accTriPerShare.add(
//                     (triReward.mul(ACC_TRI_PRECISION) / lpSupply).to128()
//                 );
//             }
...
//             int256(amount.mul(pool.accTriPerShare) / ACC_TRI_PRECISION)
//         );
...
//         PoolInfo memory pool = updatePool(pid);
//         UserInfo storage user = userInfo[pid][msg.sender];
//         int256 accumulatedTri = int256(
//             user.amount.mul(pool.accTriPerShare) / ACC_TRI_PRECISION
//         );
...
//         // Effects
//         user.rewardDebt = accumulatedTri.sub(
//             int256(amount.mul(pool.accTriPerShare) / ACC_TRI_PRECISION)
//         );
//         user.amount = user.amount.sub(amount);
